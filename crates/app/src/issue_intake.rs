//! W1TN3SS manual "Report an issue" intake — the C0PL4ND host glue.
//!
...
//! yields a clipboard body the host copies via egui, or offers a `mailto:`
//! fallback to a config-injected support alias. All of the URL / clipboard-body
//! / mailto building is DELEGATED to `itasha_report_core::intake` (the consumed
//! seam, pinned by git tag); this module only wires the dialog UX state and the
//! launch decision.
//!
//! C0PL4ND has no `arboard` dependency, so the clipboard fallback returns the
//! body text for the host to copy through egui's `ctx.copy_text` at the call
//! site (rather than the SDK/`arboard` copying internally) — keeping this module
...
//!   and presses Open / Copy / Email. There is no background or default-on path.
...
/// C0PL4ND's eframe stack is always the wgpu backend, so this is a static,
/// non-identifying string — never a GPU device name, vendor ID, or host value.
...
/// the browser / copies via egui's clipboard accordingly — the module itself
/// never touches the OS clipboard (C0PL4ND has no `arboard`; egui owns it).
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum IntakeAction {
    /// Open this URL in the browser (the SDK already launched it — the URL fit).
    Opened(IntakeOutcome),
    /// Copy this text to the clipboard via egui `ctx.copy_text`, then the host
    /// reports [`IntakeOutcome::CopiedToClipboard`].
...
/// [`IntakeAction::CopyToClipboard`] with the body the host copies via egui.
///
...
pub fn open_or_copy(req: &IssueFormRequest) -> IntakeAction {
    ...
    if url.len() <= GITHUB_URL_LENGTH_THRESHOLD {
        ...
        // URL would 414 — copy the body for a manual paste.
