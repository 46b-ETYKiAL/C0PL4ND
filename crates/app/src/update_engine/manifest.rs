...
//! (SHA-256) and AUTHENTICITY (minisign against the embedded key). The signed
//! manifest adds a layer the per-asset gates cannot: a single, signed,
...
//!
//! ```json
//! { "schema":"itasha.update.manifest/v1","product":"c0pl4nd","version":"0.4.9",
//!   "release_index":4009,"minimum_version":"0.4.0",
//!   "published_utc":"2026-06-29T14:17:42Z","valid_until_utc":"2026-07-13T14:17:42Z",
//!   "assets":[ {"platform":"x86_64-pc-windows-msvc","kind":"zip",
//!     "asset_name":"c0pl4nd-v0.4.9-x86_64-pc-windows-msvc.zip",
//!     "url":"https://github.com/.../c0pl4nd-...zip","size":8095481,
...
/// The schema-id prefix every C0PL4ND update manifest carries. The Tier-1
...
pub const MANIFEST_PRODUCT: &str = "c0pl4nd";
...
pub struct ManifestAsset {
    ...
    /// or `"exe"` (the setup installer — fresh installs only, NEVER an in-place
    /// update target). [`Manifest::archive_for`] selects only archive kinds.
    #[serde(default)]
    pub kind: String,
...
impl Manifest {
    ...
    /// contains `target` (robust to the tag prefix in the name); AND its `kind`
    /// is an archive (`"zip"` / `"tar.gz"`, NEVER `"exe"` — the setup installer
    /// is for fresh installs, never an in-place swap); AND its `asset_name` ends
    /// with `ext` (`.zip` on Windows, `.tar.gz` on Unix). Returns the FIRST
...
/// installer) is deliberately excluded — it is a fresh-install artifact and
/// must never be selected as an in-place update target.
...
mod tests {
    ...
    fn fixture_json(version: &str, release_index: u64, valid_until: &str) -> String {
        format!(
            r#"{{"schema":"itasha.update.manifest/v1","product":"c0pl4nd","version":"{version}",
"release_index":{release_index},"minimum_version":"0.4.0",
"published_utc":"2026-06-29T14:17:42Z","valid_until_utc":"{valid_until}",
"assets":[
 {{"platform":"x86_64-pc-windows-msvc","kind":"zip",
  "asset_name":"c0pl4nd-v{version}-x86_64-pc-windows-msvc.zip",
  "url":"https://github.com/o/r/releases/download/v{version}/c0pl4nd-v{version}-x86_64-pc-windows-msvc.zip",
  "size":8095481,"sha256":"1963210d0eb0f510"}},
 {{"platform":"x86_64-unknown-linux-gnu","kind":"tar.gz",
  "asset_name":"c0pl4nd-v{version}-x86_64-unknown-linux-gnu.tar.gz",
  ...
  "size":9000000,"sha256":"00000000feedface"}}
    ...
    fn archive_for_skips_exe_and_picks_the_matching_archive() {
        ...
        let win = m
            ...
            .expect("a windows zip archive must be selected");
        assert_eq!(win.kind, "zip");
        assert!(win.asset_name.ends_with(".zip"));
        assert!(
    ...
    fn archive_for_never_selects_an_exe_even_when_only_exe_exists() {
        ...
        let m = Manifest {
            assets: vec![ManifestAsset {
                platform: "x86_64-pc-windows-msvc".to_string(),
                kind: "exe".to_string(),
                asset_name: "c0pl4nd-setup-v0.4.9.exe".to_string(),
                ..Default::default()
            }],
            ..Default::default()
        };
        assert!(m.archive_for("x86_64-pc-windows-msvc", ".exe").is_none());
        assert!(m.archive_for("x86_64-pc-windows-msvc", ".zip").is_none());
    }
    ...
    fn archive_for_matches_on_asset_name_substring_when_platform_blank() {
        ...
        // triple + the right extension still matches (mirrors net::select_update).
        let m = Manifest {
            assets: vec![ManifestAsset {
                platform: String::new(),
                kind: "tar.gz".to_string(),
                asset_name: "c0pl4nd-v1.0.0-x86_64-unknown-linux-gnu.tar.gz".to_string(),
                ..Default::default()
            }],
            ..Default::default()
        };
    ...
    fn unknown_fields_are_tolerated_forward_compat() {
        ...
        let json = r#"{"schema":"itasha.update.manifest/v2","product":"c0pl4nd",
